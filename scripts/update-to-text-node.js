...
async function updateChatbotToCorrectType() {
    try {
        console.log('🔧 Updating chatbot to use correct TEXT_NODE type...');
        ...
        for (const chatbot of chatbots) {
            ...
            try {
                ...
                // Update all nodes to use TEXT_NODE (correct enum value)
                Object.keys(botConfig).forEach(nodeId => {
                    const node = botConfig[nodeId];
                    if (node.type === 'TEXT_MESSAGE_NODE' || node.type === 'START_NODE') {
                        console.log(`   Updating node ${nodeId} from ${node.type} to TEXT_NODE`);
                        node.type = 'TEXT_NODE';
                        updated = true;
                    }
                });
        ...
        console.log('\n🎉 Chatbot type update completed!');
        
    } catch (error) {
        console.error('❌ Error updating chatbot types:', error);
    } finally {
