...
export default function EditWhatsAppWidgetPage(props: IProps) {
  const user = props.user ? JSON.parse(props.user) : {};
  const router = useRouter();
  const { id, workspaceId: workspaceIdFromQuery } = router.query as { id?: string; workspaceId?: string };
  const { id: workspaceIdFromStore } = useSelector(getCurrentWorkspace);
  const effectiveWorkspaceId = (workspaceIdFromQuery as string) || (workspaceIdFromStore ? String(workspaceIdFromStore) : '');
  ...
  const selectedIconOpt = iconOptions.find((i) => i.value === chatIcon);
  const SelectedIcon: any = selectedIconOpt?.icon;

  useEffect(() => {
    const fetchWidget = async () => {
      if (!id || !effectiveWorkspaceId) return;
      try {
        setIsLoading(true);
        const response = await axios.get(`/api/whatsapp-widget/${id}?workspaceId=${effectiveWorkspaceId}`);
        const widget = response.data?.data as WidgetData;
        if (widget) {
          setWidgetData(widget);
          setBrandName(widget.name || '');
          const digits = (widget.phoneNumber || '').replace(/\D/g, '');
          const local = digits.startsWith('234') ? digits.slice(3) : digits;
          setPhone(local);
          setPreFilledMsg(widget.message || '');
          setPagePosition(widget.position === 'right' ? 'bottom-right' : 'bottom-left');
          setBrandColor(widget.backgroundColor || '#25D366');
          setChatIcon(widget.icon || 'whatsapp');
        }
      } catch (error: any) {
        const status = error.response?.status;
        if (status === 404) toast.error('Widget not found');
        else if (status === 401) toast.error('Unauthorized to access this widget');
        else toast.error('Failed to load widget');
        router.push('/dashboard/automations/whatsapp-widget');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const validatePhoneNumber = (val: string): boolean => /^[0-9]{10,15}$/.test(val);
  const validateColor = (val: string): boolean => /^#([0-9A-F]{3}){1,2}$/i.test(val);

  const validateForm = (): { isValid: boolean; message?: string } => {
    if (!brandName?.trim()) return { isValid: false, message: 'Brand name is required' };
    if (!phone?.trim()) return { isValid: false, message: 'Phone number is required' };
    if (!validatePhoneNumber(phone)) return { isValid: false, message: 'Please enter a valid phone number (10-15 digits)' };
    if (!brandColor || !validateColor(brandColor)) return { isValid: false, message: 'Please select a valid brand color' };
    if (!bubbleText?.trim()) return { isValid: false, message: 'Chat bubble text is required' };
    if (!welcomeMsg?.trim()) return { isValid: false, message: 'Welcome message is required' };
    return { isValid: true };
  };
  ...
  const handleCopy = async () => {
    ...
    try {
      ...
      if (copyCheck) {
        ...
        setTimeout(() => copyCheck.classList.add('hidden'), 2000);
      }
    } catch {
  ...
  const handleUpdateWidget = async (e: React.FormEvent) => {
    ...
    if (!isValid) { toast.error(message || 'Please fill in all required fields'); return; }
    ...
    try {
      const payload = {
        ...
        textColor: '#FFFFFF',
        icon: chatIcon,
        workspaceId: Number(effectiveWorkspaceId),
      };

      const res = await axios.put(`/api/whatsapp-widget/${id}?workspaceId=${effectiveWorkspaceId}`, payload);
      setWidgetData(res.data?.data);
      toast.success('Widget updated successfully!');
      setShowEmbedModal(true);
    } catch (error: any) {
      const errorMessage = error.response?.data?.message || error.message || 'Failed to update widget. Please try again.';
  ...
  return (
    <DashboardLayout user={user}>
      ...
      <div className="flex items-center gap-2">
        ...
        <h1 className="text-2xl font-bold mb-6 text-black">Edit widget</h1>
                  ...
                  <div className="flex flex-col gap-6">
                    ...
                    <label className="flex items-center cursor-pointer group">
                      <div className="flex items-center">
                        <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center mr-3 ${pagePosition === 'bottom-right' ? 'border-blue-500 bg-white' : 'border-gray-300 bg-white'}`}>
                          {pagePosition === 'bottom-right' && (<div className="w-2.5 h-2.5 rounded-full bg-blue-500"></div>)}
                        </div>
                        <input type="radio" checked={pagePosition === 'bottom-right'} onChange={() => setPagePosition('bottom-right')} className="hidden" />
                    ...
                    <label className="flex items-center cursor-pointer group">
                      <div className="flex items-center">
                        <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center mr-3 ${pagePosition === 'bottom-left' ? 'border-blue-500 bg-white' : 'border-gray-300 bg-white'}`}>
                          {pagePosition === 'bottom-left' && (<div className="w-2.5 h-2.5 rounded-full bg-blue-500"></div>)}
                        </div>
                        <input type="radio" checked={pagePosition === 'bottom-left'} onChange={() => setPagePosition('bottom-left')} className="hidden" />
                        <div className="relative w-20 h-12 bg-gray-50 border border-gray-200 rounded-lg mr-3 shadow-sm">
                    ...
                    {iconOptions.filter((opt) => opt.value !== 'dropdown').map((opt) => (
                      <div key={opt.value} className="flex items-center gap-3">
                        <label className="flex items-center cursor-pointer">
                          <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${chatIcon === opt.value ? 'border-blue-500 bg-white' : 'border-gray-300 bg-white'}`}>
                            {chatIcon === opt.value && (<div className="w-2.5 h-2.5 rounded-full bg-blue-500"></div>)}
                          </div>
                          <input type="radio" checked={chatIcon === opt.value} onChange={() => setChatIcon(opt.value)} className="hidden" />
                        </label>
                        <div className={`w-12 h-12 flex items-center justify-center rounded-full border-2 transition-all duration-150 cursor-pointer ${chatIcon === opt.value ? 'border-green-500' : 'border-gray-200'}`} style={{ background: chatIcon === opt.value ? '#25D366' : '#F5F5F5' }} onClick={() => setChatIcon(opt.value)}>
                          {opt.icon && React.createElement(opt.icon as any, { size: 24, color: '#fff' })}
                        </div>
                        {opt.value === 'custom' && (
                          <button type="button" className="px-1 py-2 bg-blue-600 text-white text-sm rounded-lg hover:bg-blue-700 transition-colors duration-200">Set custom avatar</button>
                        )}
                      </div>
                    ))}
                  </div>
          ...
          <div className="flex-1 flex flex-col items-center mt-8 md:mt-0">
            <div className="bg-white rounded-xl shadow p-8 min-w-[340px] max-w-sm mx-auto">
              ...
              <div className="relative bg-gray-100 rounded-lg border border-gray-200 overflow-hidden" style={{ width: '300px', height: '280px' }}>
                ...
                <div className="absolute" style={{ [pagePosition.includes('right') ? 'right' : 'left']: '12px', bottom: '12px' }}>
                  {/* Chat Bubble - shown when preview is open */}
                  {previewOpen && (
                    <div className="mb-12 relative">
                      <div className="bg-white rounded-2xl p-4 shadow-lg border-2 border-black relative" style={{ width: '180px' }}>
                        {/* Speech bubble tail */}
                        <div className="absolute -bottom-2 w-4 h-4 bg-white border-r-2 border-b-2 border-black transform rotate-45" style={{ [pagePosition.includes('right') ? 'right' : 'left']: '16px' }}></div>
                        {/* Icon */}
                        <div className="w-8 h-8 rounded-full flex items-center justify-center mb-3" style={{ backgroundColor: brandColor }}>
                          {SelectedIcon ? <SelectedIcon size={16} color="#fff" /> : (
                        ...
                        {/* Heading */}
                        <h3 className="text-sm font-bold text-gray-900 mb-3 leading-tight">Got any questions?<br />we are here to help</h3>
                        {/* Chat Button */}
                        <button className="w-full text-white text-xs font-medium py-2 px-3 rounded-lg flex items-center justify-between mb-2" style={{ backgroundColor: brandColor }}>
                          <span>{bubbleText || 'Chat with us'}</span>
                          <span>→</span>
                        </button>
                        {/* Footer */}
                        <div className="flex items-center gap-1 text-gray-400 text-xs">
                          <svg width="8" height="8" fill="none" viewBox="0 0 16 16"><path d="M2.5 13.5L13.5 2.5M13.5 2.5L10.5 2M13.5 2.5L14 5.5" stroke="#9ca3af" strokeWidth="1.2" strokeLinecap="round" /></svg>
                          Magic by wendi.com
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="flex justify-end">
                    <button className="w-12 h-12 rounded-full shadow-lg flex items-center justify-center cursor-pointer focus:outline-none transform transition-transform hover:scale-105" style={{ backgroundColor: brandColor }} onClick={() => setPreviewOpen(!previewOpen)}>
                      {previewOpen ? (
                        <X size={20} color="#fff" />
                      ) : SelectedIcon ? (
              ...
              <div className="mt-4 p-3 bg-gray-50 rounded-lg text-sm">
                <div className="flex justify-between items-center mb-2"><span className="text-gray-600">Position:</span><span className="font-medium">{pagePosition.replace('-', ' ')}</span></div>
                <div className="flex justify-between items-center mb-2"><span className="text-gray-600">Color:</span><div className="flex items-center gap-2"><div className="w-4 h-4 rounded-full border" style={{ backgroundColor: brandColor }}></div><span className="font-medium">{brandColor}</span></div></div>
                <div className="flex justify-between items-center"><span className="text-gray-600">Message:</span><span className="font-medium text-right max-w-[150px] truncate">{bubbleText || 'Chat with us'}</span></div>
              </div>
            </div>
          </div>
        </div>
          ...
          <form onSubmit={handleUpdateWidget}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <div className="flex gap-2"><span className="inline-flex items-center px-2 border rounded-l bg-gray-50 text-gray-600">+234</span><input type="tel" className="w-full border rounded-r px-3 py-2" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="Phone number" /></div>
              </div>
              <div>
              ...
              <div>
                ...
                <div className="flex gap-4 mt-1"><label className="flex items-center gap-1"><input type="radio" checked={openByDefault} onChange={() => setOpenByDefault(true)} /> True</label><label className="flex items-center gap-1"><input type="radio" checked={!openByDefault} onChange={() => setOpenByDefault(false)} /> False</label></div>
              </div>
              <div>
            ...
            <button type="submit" className="mt-8 w-full bg-blue-600 text-white py-3 rounded-lg font-semibold hover:bg-blue-700 transition flex items-center justify-center gap-2" disabled={isUpdating}>
              {isUpdating ? (<><svg className="animate-spin -ml-1 mr-2 h-4 w-4 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>Updating...</>) : ('Update Widget')}
            </button>
          </form>
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40 p-4" onClick={() => setShowEmbedModal(false)}>
              <div className="bg-white rounded-2xl shadow-xl p-8 max-w-lg w-full relative" onClick={e => e.stopPropagation()}>
                <div className="flex items-start justify-between mb-4">
                  <div>
                    <h2 className="text-xl font-bold text-gray-900">Widget Updated</h2>
                    <p className="text-gray-500 mt-1">Add this code to your website to display the WhatsApp widget</p>
                  </div>
                  <button onClick={() => setShowEmbedModal(false)} className="text-gray-400 hover:text-gray-600 transition-colors" aria-label="Close"><X size={20} /></button>
                </div>
                <div className="mb-6">
                  <div className="flex items-center justify-between mb-2"><label className="text-sm font-medium text-gray-700">Embed Code</label><button onClick={handleCopy} className="text-sm text-blue-600 hover:text-blue-800 flex items-center gap-1.5 transition-colors" type="button"><Check id="copy-check" size={16} className="text-green-500 hidden" /><span>Copy Code</span></button></div>
                  <div className="relative">
                    <textarea ref={codeRef} readOnly value={embedCode} className="w-full bg-gray-900 border border-gray-700 rounded-lg p-4 font-mono text-sm text-white resize-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition" rows={8} onClick={(e) => (e.target as HTMLTextAreaElement).select()} spellCheck={false} style={{ lineHeight: '1.5' }} />
                  </div>
                </div>
                <div className="bg-blue-50 p-4 rounded-lg mb-6 border border-blue-100">
                  <h3 className="font-medium text-blue-800 mb-2 flex items-center"><MessageSquare className="w-4 h-4 mr-2" />How to use:</h3>
                ...
                <div className="flex flex-col sm:flex-row justify-end gap-3">
                  <button type="button" onClick={() => setShowEmbedModal(false)} className="px-4 py-2.5 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 transition-colors font-medium">Close</button>
                  <Link href="/dashboard/automations/whatsapp-widget" className="px-4 py-2.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors font-medium text-center block">View All Widgets</Link>
                </div>
              </div>
            </div>
          )}
