...
const DashboardLayout = (props: IProps) => {
    ...
    // Validate and set workspace with ownership check
    ...
    useEffect(() => {
        ...
        // Check if current workspace belongs to this user
        const workspaceBelongsToUser = userWorkspaces.some(
            ws => ws.id === currentWorkspace?.id
        );

        // If workspace exists but doesn't belong to user, reset it
        if (currentWorkspace && currentWorkspace.id > 0 && !workspaceBelongsToUser) {
            console.log('Workspace belongs to different user, resetting to user\'s first workspace');
            if (userWorkspaces.length > 0) {
                dispatch(setCurrentWorkspace(userWorkspaces[0]));
                hasInitializedWorkspace.current = true;
            }
        }
        // If no workspace is selected, set the first one
        else if ((!currentWorkspace || currentWorkspace.id === 0) && userWorkspaces.length > 0) {
        ...
        // Workspace is valid and already set
        else if (currentWorkspace && currentWorkspace.id > 0 && workspaceBelongsToUser) {
            hasInitializedWorkspace.current = true;
        }
                        ...
                        {navs.slice(0, -2).map((item, index) => (
                            <li key={index}>
                                <a
                                    ...
                                    className={`group flex items-center p-2 rounded-lg transition-colors ${router.pathname === item.link
                                        ...
                                        >
                                            {Array.isArray(allWorkspace) && allWorkspace.map((workspace: Workspace, index: number) => (
                                                <div
                                                    ...
                                                    className={`px-4 py-2.5 text-sm cursor-pointer transition-colors ${workspace.id === currentWorkspace?.id
            ...
            <div className="md:hidden fixed bottom-0 left-0 right-0 h-16 bg-white border-t border-gray-200 flex items-center justify-around">
                {navs.slice(0, 4).map((nav, index) => (
                    <button
                        ...
                        className={`p-2 rounded-full ${router.pathname === nav.link
